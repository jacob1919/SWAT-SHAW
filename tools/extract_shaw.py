...
def main():
    ...
    # A negative value preserves the official shared slope. Nonnegative values
    # override radiation geometry only; hydraulic/interception slopes stay intact.
    common['RADOPT'] = [('RADIATION_SLOPE','REAL','',2)]
    ...
    for name, unit in selected.items():
        ...
        for st in unit['statements']:
            ...
            text = ''.join(chunks)
            if name == 'SOLAR' and re.match(r'\s*COMMON\s*/SHP_TIMEWT/',text,re.I):
                body.append(emit('COMMON /SHP_RADOPT/ SHP_RADIATION_SLOPE'))
                body.append(emit('REAL SHP_RADIATION_SLOPE, SHP_EFFECTIVE_SLOPE'))
            if name == 'SOLAR' and re.match(r'\s*SUNSLP\s*=\s*ASIN',text,re.I):
                body.append(emit('SHP_EFFECTIVE_SLOPE=SLOPE'))
                body.append(emit('IF (SHP_RADIATION_SLOPE.GE.0.) SHP_EFFECTIVE_SLOPE=SHP_RADIATION_SLOPE'))
                text=re.sub(r'\bSLOPE\b','SHP_EFFECTIVE_SLOPE',text,flags=re.I)
                radiation_slope_sites += 1
    ...
    assert radiation_slope_sites == 1, radiation_slope_sites
    ...
    if moved_data:
        ...
        body.append(emit('REAL SHP_RADIATION_SLOPE'))
        body.append(emit('COMMON /SHP_RADOPT/ SHP_RADIATION_SLOPE'))
        body.append(emit('DATA SHP_RADIATION_SLOPE /-1.0/'))
        body.append(emit('REAL SHP_CANOPY_WATER_TOL'))
        body.append(emit('INTEGER SHP_CANOPY_JACOBIAN'))
        body.append(emit('COMMON /SHP_OPTIONS/ SHP_CANOPY_WATER_TOL, SHP_CANOPY_JACOBIAN'))
    ...
    manifest={'official_source':'https://www.ars.usda.gov/pacific-west-area/boise-id/northwest-watershed-research-center/docs/shaw-model/',
              ...
              'optional_radiation_slope':{'option':'radopt_radiation_slope','default':-1,
                  'meaning':'Negative preserves original shared slope; nonnegative overrides only SOLAR incidence geometry. Saved per HRU; hydraulic slope remains unchanged.'},
